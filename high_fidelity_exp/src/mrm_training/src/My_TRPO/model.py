...
class TRPO():
    def __init__(self, args, observation_size, action_size, hidden_size1 = 64, hidden_size2 = 64):   # __init__ will build the computation graph for TRPO
        ...
        # construct the policy net with two hiddenf layers
        with tf.variable_scope("policy"):
            ...
            h2 = tf.nn.tanh(h2)
            h3 = fully_connected(h2, self.hidden_size2, self.action_size, weight_init, bias_init, "policy_h3")
            action_dist_logstd_param = tf.Variable((.001*np.random.randn(1, self.action_size)).astype(np.float32), name="policy_logstd")
        ...
        self.vf.create_net(self.observation_size + 1)  # +1 for adding a bias dim to the value function net
        # self.vf = LinearVF()
        self.saver = tf.train.Saver()  # object for saving the model
