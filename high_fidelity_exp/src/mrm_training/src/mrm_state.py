...
class MrmState(object):
    ...
    def calculate_reward_distance_from_obstacle(self, weight = 1.0):
        ...
        # if obstacle_distance <= 0.02:
        #     reward = self.collision_reward
        # else:
        #     reward = 0.0
        return reward

    def calculate_reward_action(self, weight=0.001):
        """

        """
        acumulated_joint_action = 0.0
        joint_actions = self.action
        for joint_action in joint_actions:
            acumulated_joint_action += abs(joint_action)

        reward = -weight * acumulated_joint_action
        return reward


    def calculate_total_reward(self):
        r1 = self.calculate_reward_distance_from_des_point(self._weight_r1)
        # r2 = self.calculate_reward_action(self._weight_r2)
        ...
        # rospy.loginfo("r2 joint_actions=" + str(r2))
        return total_reward
    ...
    def get_observations(self):
        ...
        states = ["joint_01_states",
                 "joint_02_states",
                 "joint_03_states",
                 "prismatic_joint_states",
                 "end_effector_pos",
                 "target_pos",
                 "laser_scanner_data"]
