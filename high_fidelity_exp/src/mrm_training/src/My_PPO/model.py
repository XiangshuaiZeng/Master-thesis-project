...
class PPO():
    def __init__(self, args, observation_size, action_size, hidden_size1 = 128, hidden_size2 = 64, hidden_size3 = 32):   # __init__ will build the computation graph for TRPO
        ...
        with tf.variable_scope("policy"):
            ...
            action_dist_logstd_param = tf.Variable((.01*np.random.randn(1, self.action_size)).astype(np.float32) - 0.5, name="policy_logstd")
        ...
        self.policy_loss = surr + self.args["coef_en"] * ent

        # adam optimizer for updating parameters
        self.learning_rate = tf.placeholder(tf.float32)
        optimizer = tf.train.AdamOptimizer(self.learning_rate)

        gvs = optimizer.compute_gradients(self.policy_loss)
    ...
    def learn(self, paths):
        ...
        if kl_after > 1.5 * self.args["max_kl"]:
            self.args["lr_ph"] /= 2.0
