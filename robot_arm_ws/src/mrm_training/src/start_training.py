...
if __name__ == '__main__':
    ...
    i = 0
    for obs_coff in [1.5]:
        modeldir = outdir + '/PPO_model_new1/pos_history_%s.ckpt' % string_coff[i]
        # save the episode reward and the trained RL model
        data_dir = outdir + '/iteration_reward_pos_history_%s1.txt' % string_coff[i]
        data_dir1 = outdir + '/success_times_pos_history_%s1.txt' % string_coff[i]
        data_dir2 = outdir + '/avg_laser_pos_history_%s1.txt' % string_coff[i]
