...
if __name__ == '__main__':
    ...
    i = 0
    for obs_coff in [1.5]:
        modeldir = outdir + '/PPO_model_new1/torque_history_%s.ckpt' % string_coff[i]
        # save the episode reward and the trained RL model
        data_dir = outdir + '/iteration_reward_torque_history_%s.txt' % string_coff[i]
        data_dir1 = outdir + '/success_times_torque_history_%s.txt' % string_coff[i]
        data_dir2 = outdir + '/avg_laser_torque_history_%s.txt' % string_coff[i]
