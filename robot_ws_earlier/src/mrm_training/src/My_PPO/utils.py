...
def rollout(env, agent, max_pathlength, n_timesteps):
    paths = []
    timesteps_sofar = 0
    reach_targets = []  # for recording if the ee reaches the target
    avg_lasers = []
    while timesteps_sofar < n_timesteps:   # the overall time-steps per batch, usually a large number
        ...
        ob, avg_laser = env.reset()        # reset env and get the initial state
        for time_step in xrange(max_pathlength):
        ...
        if res[4]:     # if the robot is highly stuck in the pipe, discrad this episode
            rospy.loginfo('Highly stuck, discard the episode!!')
            continue

        reach_targets.append(res[3])
        avg_lasers.append(avg_laser)
        ...
        timesteps_sofar += (time_step + 1)
        rospy.loginfo("The time steps so far: " + str(timesteps_sofar) + "\n")
    return paths, reach_targets, avg_lasers
