...
if __name__ == '__main__':
    ...
    i = 0
    for obs_coff in [0.6]:
        modeldir = outdir + '/PPO_model_new1/pos_laser_%s.ckpt' % string_coff[i]
        # save the episode reward and the trained RL model
        data_dir = outdir + '/iteration_reward_pos_laser_%s.txt' % string_coff[i]
        data_dir1 = outdir + '/success_times_pos_laser_%s.txt' % string_coff[i]
        data_dir2 = outdir + '/avg_laser_pos_laser_%s.txt' % string_coff[i]
