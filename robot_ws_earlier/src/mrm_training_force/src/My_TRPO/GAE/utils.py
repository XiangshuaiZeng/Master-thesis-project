...
def FLAT_GRAD(loss, vrbs):
	...
	return tf.concat([tf.reshape(g, [np.prod(v.get_shape().as_list()),]) for (g, v) in zip(grads, vrbs)], 0)
...
class GetValue:
	def __init__(self, sess, variable_list, name=None):
		self.name = name
		self.sess = sess
		self.op_list = tf.concat([tf.reshape(v, [np.prod(v.get_shape().as_list())]) for v in variable_list], 0)
