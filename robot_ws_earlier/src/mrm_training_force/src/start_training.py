...
if __name__ == '__main__':
    ...
    "vf_constraint" : 0.01,
    "max_kl" : .01,
    "coef_en": 0.02,  # coefficient for entropy bonus, only used in PPO
    ...
    i = 0
    for obs_coff in [0.2]:
        modeldir = outdir + '/PPO_model_new1/torque_laser_%s.ckpt' % string_coff[i]
        # save the episode reward and the trained RL model
        data_dir = outdir + '/iteration_reward_torque_laser_%s.txt' % string_coff[i]
        data_dir1 = outdir + '/success_times_torque_laser_%s.txt' % string_coff[i]
        data_dir2 = outdir + '/avg_laser_torque_laser_%s.txt' % string_coff[i]
        env.mrm_state_object._weight_r3 = obs_coff
        # env.mrm_state_object.collision_reward = obs_coff
        ...
        for iteration in range(args['max_iteration']):
            ...
            while True:
                ...
                rollouts, reach_targets, avg_lasers = rollout(env, agent, args["timesteps_per_episode"], args["timesteps_per_batch"])   # collect data
                ...
                lasers_dis.append(np.mean(avg_lasers))
                ...
                np.savetxt(data_dir2, lasers_dis, fmt='%f')
